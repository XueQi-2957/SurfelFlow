    ...
    def draw_tracks_on_video(
        ...
        if self.mode == "optical_flow":
            ...
            if self.mode == "rainbow":
                ...
                still_length = int(still_length)
                still_length = max(0, min(still_length, N))

                dynamic_start = still_length
                dynamic_count = N - dynamic_start

                if dynamic_count > 0:
                    dynamic_y = tracks[query_frame, dynamic_start:, 1]
                    y_move_min, y_move_max = dynamic_y.min(), dynamic_y.max()
                    move_norm = plt.Normalize(y_move_min, y_move_max)

                    for n in range(dynamic_start, N):
                        color = self.color_map(move_norm(tracks[query_frame, n, 1]))
                        color = np.array(color[:3])[None] * 255
                        vector_colors[:, n] = np.repeat(color, T, axis=0)
                else:
                    print(f"[WARN] No dynamic tracks to color: still_length={still_length}, N={N}")   
                #if still_length > 0 and still_length < T:
