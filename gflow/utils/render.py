...
def render_multiple(input_group, 
    ...
    # ``msplat.alpha_blending`` exposes the accumulated feature value but not
    # the accumulated opacity.  Rendering a unit feature with a zero
    # background recovers that opacity exactly and lets post-training
    # evaluators form an opacity-normalized (expected) depth map.  These
    # branches are opt-in so the default training return types and their
    # memory footprint remain unchanged.
    geometry_depth_requested = any(
        name in return_type for name in ("alpha", "depth_map_accum", "depth_expected")
    )
    rendered_alpha = None
    if geometry_depth_requested:
        alpha_values = torch.ones_like(depth)
        rendered_alpha = msplat.alpha_blending(
            uv,
            conic,
            opacity,
            alpha_values,
        ...
        if "alpha" in return_type:
            return_dict["alpha"] = rendered_alpha

    if "depth_map_accum" in return_type or "depth_expected" in return_type:
        rendered_depth_accum = msplat.alpha_blending(
            uv,
            conic,
            opacity,
            depth,
        ...
        if "depth_map_accum" in return_type:
            return_dict["depth_map_accum"] = rendered_depth_accum
        if "depth_expected" in return_type:
            denominator = rendered_alpha.clamp_min(1e-8)
            return_dict["depth_expected"] = torch.where(
                rendered_alpha > 1e-8,
                rendered_depth_accum / denominator,
                torch.zeros_like(rendered_depth_accum),
            )

    if "depth_map_color" in return_type:
        # apply colormap
        depth_color = apply_float_colormap(
