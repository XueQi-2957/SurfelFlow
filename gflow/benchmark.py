...
def main(
    ...
    # 路径自动解析：若log_path下没有ckpt/目录（如传入的是0_latest这类封装目录），
    # 则自动找其中最新的子目录（包括symlink指向的目录）作为真实log_path
    if not os.path.isdir(os.path.join(log_path, "ckpt")):
        sub_entries = sorted([
            e for e in os.scandir(log_path)
            if e.is_dir(follow_symlinks=True)
        ], key=lambda e: e.name)
        if sub_entries:
            resolved = sub_entries[-1].path  # 按名称排序取最新
            print(f"[benchmark] log_path '{log_path}' 下无ckpt/目录，自动解析为: {resolved}")
            log_path = resolved
        else:
            raise FileNotFoundError(f"[benchmark] 无法在 '{log_path}' 下找到有效的log子目录")
    csv_dir = {}
    ...
    if eval_recon:
        ...
        # 预先扫描并按字母集索引排序场景下的所有潜在真值图片 (GT)
        valid_exts = ('*.jpg', '*.png', '*.jpeg', '*.JPG', '*.PNG')
        all_gt_paths = []
        for ext in valid_exts:
            all_gt_paths.extend(glob.glob(os.path.join(sequence_path, ext)))
        all_gt_paths = sorted(all_gt_paths)
        ...
        for image_path in tqdm(log_image_paths):
            ...
            idx_str = base_name.split(".")[0].split("_")[-1]
            
            # 使用提取到的渲染帧序号直接索引物理文件，无视原始图片的名称和后缀格式
            try:
                target_idx = int(idx_str)
                image_gt_path = all_gt_paths[target_idx]
            except (ValueError, IndexError):
                raise FileNotFoundError(f"Cannot find ground truth image corresponding to index {idx_str} in {sequence_path}")
            """4dgs"""
    ...
    if eval_seg:
        ...
        # recursively search for all images in the log path
        # 预先扫描并排序提取的分割真值掩膜结果
        epipolar_dir = sequence_path + "_epipolar"
        valid_seg_exts = ('*.jpg', '*.png', '*.jpeg', '*.JPG', '*.PNG')
        all_seg_gt_paths = []
        for ext in valid_seg_exts:
            all_seg_gt_paths.extend(glob.glob(os.path.join(epipolar_dir, ext)))
        all_seg_gt_paths = sorted(all_seg_gt_paths)
        ...
        for seg_path in tqdm(log_seg_paths):
            ...
            try:
                target_idx = int(idx_str)
                seg_gt_path = all_seg_gt_paths[target_idx]
            except (ValueError, IndexError):
                # 尝试通配符兜底逻辑 (处理可能存在的前缀)
                search_pattern = os.path.join(epipolar_dir, f"*{idx_str}_open.png")
                potential_paths = glob.glob(search_pattern)
                if not potential_paths:
                    raise FileNotFoundError(f"Cannot find segmentation GT image for index {idx_str} in {epipolar_dir}")
                seg_gt_path = potential_paths[0]
            # 添加 resize 确保它与训练输出时图片的大小 (seg0.shape[0]) 相匹配
            seg_gt = image_path_to_tensor(seg_gt_path, resize=seg0.shape[0]) # (H, W)
            ...
            # 确保分割 mask 是严格的 2D (H, W)，
            # 避免 PIL Image.fromarray 遇到 (H,W,1) 时报 TypeError
            if seg0_np.ndim > 2:
                seg0_np = seg0_np[..., 0]
            if seg_gt_np.ndim > 2:
                seg_gt_np = seg_gt_np[..., 0]
