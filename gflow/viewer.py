...
def main(args):
    ...
    bg = 0. if args.background == "black" else 1.
    render_backend = None
    if args.use_2dgs:
        import utils.render_2dgs as render_2dgs
        render_backend = render_2dgs.render_multiple_2dgs
        print("[viewer] Using 2DGS renderer")
    else:
        render_backend = render.render_multiple
        print("[viewer] Using legacy 3DGS renderer")
    ...
    while True:
        for client in server.get_clients().values():
            ...
            rendered_dict = render_backend(current_input_group, ["rgb"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use-2dgs", action="store_true", help="render checkpoints with the 2DGS backend"
    )
    parser.add_argument(
        "--background", choices=["black", "white"], default="white", help="viewer background color"
    )
