...
def main(
    num_points: int = 1000,
    sequence_path: Optional[Path] = Path("./data/davis/beauty_0/beauty_0"),
    ...
    lambda_flow: float = 0.,
    lambda_refresh: float = 0.0,
    lambda_stale_opacity: float = 0.0,
    ...
    w_distortion: float = 0.0,
    normal_start_frac: float = 0.5,
    dist_start_frac: float = 0.3,
    center_scale_world: float = 5e-4,
    init_2dgs_first_frame: bool = True,
    low_memory_2dgs: bool = False,
    legacy_densify_lr_after_densify: bool = False,
    force_split_diagnostics: bool = False,
    stale_opacity_hard_cap: float = 1.0,
    orphan_move_opacity_hard_cap: float = 1.0,
) -> None:
    ...
    def extend_optimize_frames(dst, src):
        if use_2dgs and low_memory_2dgs:
            dst.extend(src[-1:])
        else:
            dst += src
    ...
    if frame_range is None or frame_range == -1:
    ...
    if move_masks and len(move_masks) < len(img_paths):
        move_masks.extend([move_masks[-1]] * (len(img_paths) - len(move_masks)))
    
    
    start_time = time.time()
    total_following_frames = max(0, len(img_paths) - 1)
    total_train_iterations = iterations_first
    if camera_first:
        total_train_iterations += total_following_frames * iterations_camera
    if iterations_after > 0:
        total_train_iterations += total_following_frames * iterations_after
    completed_train_iterations = 0
    save_name0 = os.path.splitext(os.path.basename(img_paths[0]))[0]
    gt_image0 = utils.image_path_to_tensor(img_paths[0], resize=resize, blur=blur)
    H, W = gt_image0.shape[:2]
    raw_H, raw_W = imageio.imread(img_paths[0]).shape[:2]
    camera_focal = focal
    camera_pp = pp
    if resize is not None:
        scale_x = W / raw_W
        scale_y = H / raw_H
        camera_focal = [focal * scale_x, focal * scale_y]
        camera_pp = [pp[0] * scale_x, pp[1] * scale_y]
    gt_depth0 = utils.read_depth(img_depth_paths[0], resize=resize, depth_scale=depth_scale, depth_offset=depth_offset).unsqueeze(-1)
    if not move_masks:
        move_masks = [torch.zeros((H, W), dtype=torch.bool) for _ in img_paths]
    ...
    trainer = SimpleGaussian(gt_image=gt_image0, gt_depth=gt_depth0, num_points=num_points, 
                             ...
                             use_2dgs=use_2dgs, w_normal_consistency=w_normal_consistency, w_distortion=w_distortion,
                             normal_start_frac=normal_start_frac, dist_start_frac=dist_start_frac,
                             center_scale_world=center_scale_world,
                             init_2dgs_first_frame=init_2dgs_first_frame,
                             low_memory_2dgs=low_memory_2dgs,
                             legacy_densify_lr_after_densify=legacy_densify_lr_after_densify,
                             force_split_diagnostics=force_split_diagnostics)
    trainer.load_camera(focal=camera_focal, pp=camera_pp)
    if load_extr:
    ...
    global_train_start_time = time.time()
    frames, frames_center, frames_depth, still_rgb_np, still_center_np, move_rgb_np, move_center_np, move_seg = trainer.train(
        ...
        save_videos=True,
        save_ckpt=save_ckpt,
        save_geometry_eval=save_geometry_eval,
        ckpt_name=save_name0,
        lambda_rgb=lambda_rgb,
        lambda_depth=lambda_depth,
        lambda_refresh=lambda_refresh,
        lambda_stale_opacity=lambda_stale_opacity,
        ...
        global_step_offset=completed_train_iterations,
        global_total_steps=total_train_iterations,
        global_start_time=global_train_start_time,
        stale_opacity_hard_cap=stale_opacity_hard_cap,
        orphan_move_opacity_hard_cap=orphan_move_opacity_hard_cap,
    )
    completed_train_iterations += iterations_first
    extend_optimize_frames(frames_sequence_optimize, frames)
    extend_optimize_frames(frames_center_sequence_optimize, frames_center)
    extend_optimize_frames(frames_depth_sequence_optimize, frames_depth)
    ...
    if mask_exist:
        mask0 = utils.read_mask(mask_paths[0], resize=resize).to(trainer.device)
    ...
    if traj_num:
        ...
        interval = max(1, int(num_points / traj_num))
        ...
        if grid_traj:
            ...
            if len(concentrated_query_points) == 0: # then use all the points
                for i in range(stride_moving, trainer.H-stride_moving, stride_moving):
                    for j in range(stride_moving, trainer.W-stride_moving, stride_moving):
                        concentrated_query_points.append(np.array([j, i]))
        ...
        if tap_vid:
            with open(os.path.join(sequence_path, "video.pkl"), "rb") as f:
    ...
    for i, img_path in enumerate(img_paths[1:]):
        ...
        if camera_first:
            ...
            completed_train_iterations += iterations_camera
            extend_optimize_frames(frames_sequence_optimize, frames)
            extend_optimize_frames(frames_center_sequence_optimize, frames_center)
            extend_optimize_frames(frames_depth_sequence_optimize, frames_depth)
        ...
        if iterations_after > 0:
            ...
            completed_train_iterations += iterations_after
